...
public class HSBAdjustFilter extends PointFilter {
	...
	public Parameter[] getParameters() {
		...
		parameters[0] = Parameter.normalMinMaxFloat("hFactor", -1, 1, hFactor);
		parameters[1] = Parameter.normalMinMaxFloat("sFactor", -1, 1, sFactor);
		parameters[2] = Parameter.normalMinMaxFloat("bFactor", -1, 1, bFactor);
		return parameters;
	}
	
	public void setParameterAt(int index,Object parameter){
		if (index == 0)
			hFactor = ((Float)parameter).floatValue();
		if (index == 1)
			sFactor = ((Float)parameter).floatValue();
		if (index == 2)
			bFactor = ((Float)parameter).floatValue();
	}
}
